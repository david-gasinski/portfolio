            ...
            {projects.map((project, index) => (
                ...
                <Box pt={10}>
                    <IconContext.Provider key={index} value={{ size: '1.5em' }}>
                         <Card>
             <CardBody>
                     <Link pb={5} textAlign={'center'} 
                     href={project.repo}  display={'flex'}
                        flexDir={'row'}
                 ...
                 <Text  textColor={'#828282'}pt={'1em'}>{project.description}</Text>
                 <Box pt={'1em'}display={'flex'} justifyContent={'start'} flexDir={'row'}>
                     <Text pr={3}> Made with: </Text>
                     {project.tech_stack.map((tech) => (
                         <Box pr={5}>
                             <IconContext.Provider key={index} value={{ size: '1.25em' }}>
                             {tech}
